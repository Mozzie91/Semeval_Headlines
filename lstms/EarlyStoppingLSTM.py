...
class EarlyStoppingLSTM(LSTMModel):
    ...
    def fit(self, train_texts, sentiment_values):
        ...
        model = Sequential()
        model.add(Dropout(0.2, input_shape=(max_length, vector_length)))
        ...
        model.add(Bidirectional(LSTM(max_length, dropout_W=0.2, dropout_U=0.2,activation='softsign')))
        model.add(Dropout(0.5))
        model.add(Dense(1))
        ...
        model.fit(train_vectors, sentiment_values, validation_split=0.1,
                  callbacks=[early_stopping, history], nb_epoch=50)
